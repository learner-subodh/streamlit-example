...
st.image(image, use_column_width=True)
st.markdown("Out of the total events trigerred through given source screens, more than **_half_** of the events are triggered at the **_'local playlist more'_** screen. And out of the total events generated from 'local playlist more' screen, around two third of events have 'target' as 1. The **_'online playlist more'_** screen also has a good enough contribution towards 'target' = 1 events.", 
            unsafe_allow_html=False)
...
st.image(image, use_column_width=True)
st.markdown("Most songs, around **_50 percent_** to be precise, have a duration of **4 minutes**, and around**_ 95 percent_** of the songs have a duration of either **3, 4 or 5 minutes**. Also, most users prefer listening to songs with **_not so long_** duration, that is somewhat close to or around **3 to 5 minutes** of length. Both 'target' values are in **_equal_** number for all songs with different durations.", 
            unsafe_allow_html=False)

st.subheader('Distribution of Listening events')
image = Image.open('dist_listening.PNG')
st.image(image, use_column_width=True)
st.markdown("As can be observed, a lot of songs have been listened to very few number of times and very few songs have been listened to a lot of time. Thus, these songs which are listened to many times might be very **_popular_** ones either in the area where the data is collected or even globally.", 
            unsafe_allow_html=False)
